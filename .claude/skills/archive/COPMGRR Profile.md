---
name: COPMGRR-Profile
description: Use when asked to create, build, or generate a COPMGRR profile or summary JSON for a studied COPMGRR document.
---

# COPMGRR Profile Creator

## Overview

Creates a structured JSON profile file for a studied COPMGRR (Congestion Offset Payment Mechanism Bilateral Revision Request), capturing all key metadata fields extracted from the COPMGRR documents. The output file is saved under the COPMGRR's own folder in a `Quick runs` sub-folder.

## Output Location

```
Documents Database/ERCOT.MKT.RULES/COPMGRR/COPMGRR<number>/Quick runs/COPMGRR<number> Profile.json
```

- Create the `Quick runs` sub-folder if it does not already exist.
- File name format: `COPMGRR<number> Profile.json` — e.g. `COPMGRR015 Profile.json`.

## Profile Fields

Extract the following fields from the COPMGRR documents. Use `null` for any field not found.

| # | JSON Key | Source |
|---|----------|--------|
| 1 | `copmbrr_number` | COPMGRR number (integer) |
...
1. Identify the COPMGRR number from the user's request or from the document file names.
2. Read the primary COPMGRR document (e.g. `*COPMGRR-01 *.docx`) and any supporting documents in the `COPMGRR<number>/` folder.
...
5. Ensure `Quick runs/` sub-folder exists under `Documents Database/ERCOT.MKT.RULES/COPMGRR/COPMGRR<number>/`.
6. Write the JSON file to `Quick runs/COPMGRR<number> Profile.json` with 2-space indentation.
...
| Mistake | Fix |
|---------|-----|
| Saving the file in the wrong folder | Always use `COPMGRR<number>/Quick runs/`, not the COPMGRR root or project root |
| Using the wrong file name | Must be `COPMGRR<number> Profile.json`, not `profile.json` or `COPMGRRProfile.json` |
