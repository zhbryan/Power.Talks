---
name: NPRR-Summarization-and-Timeline
description: Use when asked to summarize an NPRR, analyze its background and impacts, or produce a stakeholder timeline report for an NPRR document set.
---

# NPRR Summarization and Timeline of Status

## Overview

Reads all documents in an NPRR folder, synthesizes the revision's main ideas and potential impacts, and reconstructs the full ERCOT stakeholder discussion timeline. Outputs a `.docx` report saved to the NPRR's `Quick runs` sub-folder.

## Output Location

```
Documents Database/ERCOT.MKT.RULES/NPRR/NPRR<number>/Quick runs/NPRR<number> Summary.docx
```
...
Each document in the NPRR folder has a distinct role. Read them all; use only what is relevant to each report section.

| Pattern in filename | Content |
|---------------------|---------|
| `-01` main doc      | Revision text, reason, business case, sponsor, protocol sections |
...
motion_lines = [s for s in strings if "motion" in s.lower() or "nprr" in s.lower()]
```
...
List all files in `Documents Database/ERCOT.MKT.RULES/NPRR/NPRR<number>/` and sort by document sequence number (the `NN` in `NPRRxx-NN`).
...
- Protocol sections revised
- Sponsor name, company, email, phone
- Market segment

If a `NPRR<number> Profile.json` already exists in `Quick runs/`, load it instead of re-extracting — it contains all fields pre-parsed.
...
| PRS Report | Key stakeholder concerns, ERCOT responses, any protocol language modifications |
...
5. Current Status
   - Plain-language statement of where the NPRR stands today
...
# Title
t = doc.add_heading(f"NPRR{n} — {title}", level=1)
...
out_path = os.path.join(folder, "Quick runs", f"NPRR{n} Summary.docx")
...
- **Executive Summary**: Write for a non-technical reader. State what rule is changing, what problem it solves, and who is affected (Market Participants, ERCOT operations, specific resource types, etc.).
...
- **Potential Impacts**: Distinguish between operational impacts (how ERCOT or MPs change their behavior), financial impacts (settlement, cost allocation), and system/IT impacts.
...
| Using only the `-01` doc and ignoring PRS/TAC reports | PRS and TAC reports contain stakeholder concerns and protocol modifications — essential for the timeline |
| Writing the timeline from filenames alone | Always read the Report documents; filenames give the date but not the outcome |
| Saving as `.docx` to the wrong path | Must be `NPRR<number>/Quick runs/NPRR<number> Summary.docx` — not the root folder |
| Installing python-docx is missing | `pip install python-docx`; win32com is included with pywin32 (`pip install pywin32`) |
