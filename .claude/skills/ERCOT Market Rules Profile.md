...
| 12 | `reason_for_revision` | The **checked** "Reason for Revision" option(s) only — an array, always read from the **`-01` main document** (the first-sequence filing). This value also feeds the front-end "Revision Type" section (`summary.background`). Extraction lives in the shared helper `reason_checkbox.extract_checked_reasons(docx_path)`, which handles all three encodings ERCOT uses for the checkbox, in order: (a) **ActiveX controls** — read each control's OLE `contents` stream and keep the box(es) whose stored value carries the count DWORD `0x80000001`; (b) **DrawingML image checkboxes** (`<a:blip>`) and (c) **legacy VML image checkboxes** (`<v:imagedata>`) — when no ActiveX controls are present, compare the per-option box images: the image repeated across most options is the empty/unchecked box, so the option whose box image differs is the marked one. Keep the actual option label text; do **not** list every option. Legacy `.doc` filings expose no readable marker → `[]`. (NPRR implements this today; other categories still list all options via `parse_reason_list`.) |
