#!/usr/bin/env python3
"""
ERCOT PGRR Profile Creator
Implements the ERCOT Market Rules Profile skill for all PGRR issues.
...
def first_nonempty_sibling(row, label_idx):
    """Return the first cell after label_idx that differs from the label."""
...
def extract_fields_from_tables(tables):
    ...
    for row in [r for tbl in tables for r in tbl]:
        for i, cell in enumerate(row):
            ...
            if not v:
                continue
            if 'pgrr title' in cl or cl.endswith('title'):
            ...
            elif 'planning guide sections' in cl or 'sections requiring' in cl:
...
def main():
    ...
    for folder_name in folders:
        ...
        try:
            ...
            out = os.path.join(quick, f"PGRR{n} Profile.json")
            ...
            print(f"[OK] PGRR{n:>4}  {status:<10}  {title[:65]}")
            ok += 1
        except Exception as e:
