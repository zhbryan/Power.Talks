#!/usr/bin/env python3
"""
ERCOT COPMGRR Profile Creator
Implements the ERCOT Market Rules Profile skill for all COPMGRR issues.
...
EXCEL_TRACKER = os.path.join(BASE_DIR, "Current List of COPMGRRs.xlsx")
...
def extract_fields_from_tables(tables):
    ...
    for row in [r for tbl in tables for r in tbl]:
        for i, cell in enumerate(row):
            ...
            if not v:
                continue
            if 'copmgrr title' in cl or cl.endswith('title'):
            ...
            elif 'copm' in cl and ('section' in cl or 'agreement' in cl):
                if not fields['governing_document_sections']:
                    fields['governing_document_sections'] = [
                        s.strip() for s in re.split(r'[;,\n]', v) if s.strip()
                    ]
            elif 'sections requiring' in cl:
                if not fields['governing_document_sections']:
                    fields['governing_document_sections'] = [
                        s.strip() for s in re.split(r'[;,\n]', v) if s.strip()
                    ]
...
def build_profile(folder, issue_num, status):
    issue_id = f"COPMGRR{issue_num:03d}"  # zero-padded 3 digits
    profile = {
        "category": CATEGORY,
        "issue_id": issue_id,
...
def main():
    ...
    for folder_name in folders:
        ...
        try:
            ...
            out = os.path.join(quick, f"COPMGRR{n:03d} Profile.json")
            ...
            print(f"[OK] COPMGRR{n:03d}  {status:<10}  {title[:65]}")
            ok += 1
        except Exception as e:
