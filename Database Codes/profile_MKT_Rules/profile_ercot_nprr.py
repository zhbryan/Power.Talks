#!/usr/bin/env python3
"""
ERCOT NPRR Profile Creator
Implements the ERCOT Market Rules Profile skill for all NPRR issues.
...
EXCEL_TRACKER = os.path.join(BASE_DIR, "Current List of NPRRs.xlsx")
...
def extract_fields_from_tables(tables):
    ...
    for row in [r for tbl in tables for r in tbl]:
        for i, cell in enumerate(row):
            ...
            if not v:
                continue
            if 'nprr title' in cl or cl.endswith('title'):
            ...
            elif 'protocol sections' in cl or 'sections requiring' in cl or 'nodal protocol' in cl:
...
def main():
    ...
    for folder_name in folders:
        ...
        try:
            ...
            out = os.path.join(quick, f"NPRR{n} Profile.json")
            ...
            print(f"[OK] NPRR{n:>4}  {status:<10}  {title[:65]}")
            ok += 1
        except Exception as e:
