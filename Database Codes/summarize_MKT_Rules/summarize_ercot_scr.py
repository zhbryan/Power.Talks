#!/usr/bin/env python3
"""
ERCOT SCR Summary Creator
Implements the ERCOT Market Rules Summarization and Timeline skill for all SCR issues.
Reads all documents in each SCR folder, builds a 5-section .docx report and a
...
# ─── SETTINGS ────────────────────────────────────────────────────────────────
BASE_DIR      = r"E:\wamp64\www\Power.Talks\Documents Database\ERCOT.MKT.RULES\SCR"
...
def ai_executive_summary(issue_id, title, revision_desc, reason, business_case, sections, status):
    prompt = f"""Write a concise executive summary (3–5 sentences, plain English, no jargon) for this ERCOT System Change Request (SCR).

Issue: {issue_id}
Title: {title or 'N/A'}
ERCOT Systems / Functional Areas Affected: {', '.join(sections) if sections else 'Not specified'}
...
Write 3–5 sentences covering: (1) what ERCOT system or IT change is being made, (2) why the change is needed, (3) who is affected and how. Note the implementation window if stated. Do not use markdown. Do not repeat the issue number in the summary."""
    ...
    try:
        ...
        return (f"This System Change Request ({issue_id}) proposes changes to "
                f"{', '.join(sections) or 'ERCOT systems'}. "
                f"{revision_desc or ''}")
...
def write_summary_docx(out_path, issue_id, title, exec_summary, issue_details,
    ...
    doc = DocxDoc()
    h = doc.add_heading(f"{issue_id} — {title or 'System Change Request'}", level=1)
...
def main():
    folders = sorted([
        ...
        if os.path.isdir(os.path.join(BASE_DIR, d)) and re.match(r'SCR\d+', d)
    ...
    try:
        for folder_name in folders:
            ...
            try:
                ...
                print(f"[OK] SCR{n:>5}  ->  {os.path.basename(out)}")
                ok += 1
            except Exception as e:
                print(f"[ERR] SCR{n}: {e}")
                err += 1
