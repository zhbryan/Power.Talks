#!/usr/bin/env python3
"""
ERCOT NPRR Summary Creator
Implements the ERCOT Market Rules Summarization and Timeline skill for all NPRR issues.
Reads all documents in each NPRR folder, builds a 5-section .docx report and a
...
# Set False to force a full rebuild of every issue. POWERTALKS_FORCE_ALL=1
# forces a full rebuild from the environment (used by the one-off AI backfill).
ONLY_STALE = os.environ.get("POWERTALKS_FORCE_ALL", "0") != "1"
...
def ai_executive_summary(issue_id, title, revision_desc, reason, business_case, sections, status):
    prompt = f"""Write a concise executive summary (3–5 sentences, plain English, no jargon) for this ERCOT Nodal Protocol Revision Request (NPRR).
...
Write 3–5 sentences covering: (1) what Nodal Protocol rule is changing, (2) why the change is needed, (3) who is affected and how. Note whether PUCT approval was required if evident. Do not use markdown. Do not repeat the issue number in the summary."""
    ...
    try:
        ...
        return (f"This Nodal Protocol Revision Request ({issue_id}) proposes changes to "
...
def write_summary_docx(out_path, issue_id, title, exec_summary, issue_details,
    ...
    doc = DocxDoc()
    h = doc.add_heading(f"{issue_id} — {title or 'Nodal Protocol Revision Request'}", level=1)
...
def status_sentence(issue_id, inferred):
    ...
    if inferred == "In ROS/TAC Review":
        return f"{issue_id} is currently under review by the PRS or TAC committee."
...
def process_issue(folder, n):
    ...
    issue_details = [
        ...
        ("Nodal Protocol Sections Revised",   '\n'.join(sections) if sections else '—'),
...
def main():
    ...
    print(f"Processing {len(folders)} NPRR folders...\n")
    ok = err = 0
    try:
        for folder_name in folders:
            ...
            try:
                ...
                print(f"[OK] NPRR{n:>4}  ->  {os.path.basename(out)}")
                ok += 1
            except Exception as e:
                print(f"[ERR] NPRR{n}: {e}")
                err += 1
    finally:
        quit_word()
