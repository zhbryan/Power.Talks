#!/usr/bin/env python3
"""
ERCOT COPMGRR Summary Creator
Implements the ERCOT Market Rules Summarization and Timeline skill for all COPMGRR issues.
Reads all documents in each COPMGRR folder, builds a 5-section .docx report and a
companion .json, saves to <ISSUE_ID>/Quick runs/<ISSUE_ID> Summary.docx/.json

Note: COPMGRR uses zero-padded 3-digit issue IDs (e.g. COPMGRR015).
...
# ─── SETTINGS ────────────────────────────────────────────────────────────────
BASE_DIR      = r"E:\wamp64\www\Power.Talks\Documents Database\ERCOT.MKT.RULES\COPMGRR"
...
def ai_executive_summary(issue_id, title, revision_desc, reason, business_case, sections, status):
    prompt = f"""Write a concise executive summary (3–5 sentences, plain English, no jargon) for this ERCOT Congestion Offset Payment Mechanism Guide Revision Request (COPMGRR).

Issue: {issue_id}
Title: {title or 'N/A'}
COPM Agreement / Guide Sections: {', '.join(sections) if sections else 'Not specified'}
...
Write 3–5 sentences covering: (1) what COPM Bilateral Agreement or guide section is changing, (2) why the change is needed, (3) who is affected and how this affects COPM settlement. Do not use markdown. Do not repeat the issue number in the summary."""
    ...
    try:
        ...
        return (f"This COPM Guide Revision Request ({issue_id}) proposes changes to "
                f"{', '.join(sections) or 'COPM Agreement sections'}. "
                f"{revision_desc or ''}")
...
def write_summary_docx(out_path, issue_id, title, exec_summary, issue_details,
    ...
    doc = DocxDoc()
    h = doc.add_heading(f"{issue_id} — {title or 'COPM Guide Revision Request'}", level=1)
...
def status_sentence(issue_id, inferred):
    ...
    if inferred == "In ROS/TAC Review":
        return f"{issue_id} is currently under review by the COPS/PRS or TAC committee."
    return f"{issue_id} has been recently posted and is pending initial committee review."

# ─── PER-ISSUE PROCESSOR ─────────────────────────────────────────────────────
def process_issue(folder, n):
    issue_id  = f"COPMGRR{n:03d}"  # zero-padded 3 digits
    quick     = os.path.join(folder, "Quick runs")
    ...
    issue_details = [
        ...
        ("COPM Agreement Sections Revised",     '\n'.join(sections) if sections else '—'),
...
def main():
    folders = sorted([
        ...
        if os.path.isdir(os.path.join(BASE_DIR, d)) and re.match(r'COPMGRR\d+', d)
    ...
    print(f"Processing {len(folders)} COPMGRR folders...\n")
    ok = err = 0
    try:
        for folder_name in folders:
            ...
            try:
                ...
                print(f"[OK] COPMGRR{n:03d}  ->  {os.path.basename(out)}")
                ok += 1
            except Exception as e:
                print(f"[ERR] COPMGRR{n:03d}: {e}")
                err += 1
    finally:
        quit_word()
